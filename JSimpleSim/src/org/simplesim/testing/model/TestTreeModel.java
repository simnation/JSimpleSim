 ...
 * domains. Agents have got a random event queue and send a dummy messages to a
 * randomly chosen other agent.
 *
 * Used to test general functionality of agent/domain interaction, message
 * routing and simulators.
 *
...
public final class TestTreeModel {
	...
	private static class AgentState implements IAgentState {
	}
	...
	private static void createTestModel() {
		...
		final AbstractPort rootPort=RootModel.getInstance().addRoutingPort();
	...
	private static void createNextModelLevel(AbstractDomain<?> domain, AbstractPort inport, AbstractPort outport,
			int[] addr, int level) {
