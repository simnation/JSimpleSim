 ...
 * <p>
 * In the phase of model building, ports are connected with each others 
 * via the {@code connectTo} method. The sending entity puts a {@code Message} on its outport.
 * The message is then routed along the established connections. A port that has no further connection is always
 * an end-point (destination of delivery). Thus, connections are always <i>directed</i> from source outport to destination inport.
 * <p> Each port must also implement its own {@code copyMessages} strategy to support 
 * message forwarding during the simulation run. 
 *
 * @see SinglePort
 * @see MultiPort
 * @see RoutingPort
...
public abstract class AbstractPort {
	 ...
	 * @exception PortConnectionException if the port is not connected with the target 
